...
当前产品工作流不再自动选曲或混音。新 enhancement plan 默认 `music.status=disabled`，最终输出为无 BGM 的画面锁定母版，用户在剪映中手工配乐。已有音乐执行器仅为旧项目保留兼容性。逐段画面美化的分析、代理、QA 和审批命令见 [逐段画面美化生产闭环](visual-treatment-production-loop.md)。
...
- 新项目默认禁用自动配乐、音乐试听和 ducking；
- 最终增强渲染保留现场对白和环境声；
- 用户在剪映中完成选曲与混音；
- 旧音乐字段和执行器保留兼容性，但不属于当前开发范围。
...
- 旧静态 illustration/sticker/callout/map/title_card 仍可用于 preview，但在没有独立审批合同前不能进入 release；正式叙事 Overlay 通过 HyperFrames effect plan、视觉 QA 和 SHA approval 进入成片。
