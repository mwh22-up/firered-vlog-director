...
- 完整来源数：3
- 稳定模式：11 条（显式 `minimum_source_support=2`）
- 聚合命令：`vlog-director aggregate-techniques`
- 当前限制：只统计 3 份完整 study；`BV19sm2BBEDd` 的重叠首日不生成新增 observation。另有 22 条模式仍为单来源，不能把开场预分析或未完成视频计入正式证据。

## 后续执行顺序

1. 执行 `BV1ETNMzEEWQ` 的完整学习流程。
2. 执行 `BV1aDHQejEAP` 的完整学习流程。
3. 执行 `BV1TNZUB4EF6` 的完整学习流程。
4. 六支全部完成后重新聚合 techniques，并复核每条稳定模式的独立来源支持数。
