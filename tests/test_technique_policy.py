...
class TechniquePolicyTests(unittest.TestCase):
    def test_formal_aggregate_exposes_only_reachable_executable_rule(self) -> None:
        ...
        self.assertEqual(policy["source_count"], 8)
        self.assertEqual(policy["minimum_source_support"], 2)
        self.assertEqual(policy["stable_pattern_count"], 25)
        self.assertEqual(policy["source_specific_pattern_count"], 51)
        self.assertEqual(
            {
                row["technique_key"]
        ...
        self.assertEqual(len(policy["guidance_patterns"]), 75)
        self.assertIn(
