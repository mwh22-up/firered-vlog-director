...
class TechniquePolicyTests(unittest.TestCase):
    def test_formal_aggregate_compiles_supported_rules_with_execution_modes(self) -> None:
        ...
        self.assertEqual(policy["source_count"], 25)
        self.assertEqual(policy["minimum_source_support"], 2)
        self.assertEqual(policy["stable_pattern_count"], 57)
        self.assertEqual(policy["source_specific_pattern_count"], 87)
        self.assertEqual(
            {
                row["technique_key"]
        ...
        self.assertEqual(len(policy["guidance_patterns"]), 141)
        self.assertIn(
