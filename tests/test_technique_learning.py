...
class TechniqueLearningTests(unittest.TestCase):
    ...
    def test_ten_formal_studies_keep_the_learning_baseline(self) -> None:
        ...
        self.assertEqual(observation_count, 176)
        self.assertEqual(len(unique_keys), 87)
        ...
        self.assertEqual(aggregate["source_count"], 10)
        self.assertEqual(aggregate["minimum_source_support"], 2)
        self.assertEqual(len(aggregate["stable_patterns"]), 32)
        self.assertEqual(len(aggregate["source_specific_patterns"]), 55)
        self.assertEqual(
            {
    ...
    def test_committed_aggregate_matches_the_ten_formal_studies(self) -> None:
        repository_root = Path(__file__).resolve().parents[1]
