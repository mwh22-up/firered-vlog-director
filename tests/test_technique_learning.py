...
class TechniqueLearningTests(unittest.TestCase):
    ...
    def test_nine_formal_studies_keep_the_learning_baseline(self) -> None:
        ...
        self.assertEqual(len(studies), 9)
        self.assertEqual(observation_count, 157)
        self.assertEqual(len(unique_keys), 86)
        ...
        self.assertEqual(aggregate["source_count"], 9)
        self.assertEqual(aggregate["minimum_source_support"], 2)
        self.assertEqual(len(aggregate["stable_patterns"]), 26)
        self.assertEqual(len(aggregate["source_specific_patterns"]), 60)
        self.assertEqual(
            {
    ...
    def test_committed_aggregate_matches_the_nine_formal_studies(self) -> None:
        repository_root = Path(__file__).resolve().parents[1]
