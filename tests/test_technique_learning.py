...
class TechniqueLearningTests(unittest.TestCase):
    ...
    def test_seven_formal_studies_keep_the_learning_baseline(self) -> None:
        ...
        self.assertEqual(len(studies), 7)
        self.assertEqual(observation_count, 120)
        self.assertEqual(len(unique_keys), 67)
        ...
        self.assertEqual(aggregate["source_count"], 7)
        self.assertEqual(aggregate["minimum_source_support"], 2)
        self.assertEqual(len(aggregate["stable_patterns"]), 23)
        self.assertEqual(len(aggregate["source_specific_patterns"]), 44)
        self.assertEqual(
            {
    ...
    def test_committed_aggregate_matches_the_seven_formal_studies(self) -> None:
        repository_root = Path(__file__).resolve().parents[1]
