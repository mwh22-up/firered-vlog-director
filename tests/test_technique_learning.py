...
class TechniqueLearningTests(unittest.TestCase):
    ...
    def test_eleven_formal_studies_keep_the_learning_baseline(self) -> None:
        ...
        self.assertEqual(len(studies), 11)
        self.assertEqual(observation_count, 197)
        self.assertEqual(len(unique_keys), 92)
        ...
        self.assertEqual(aggregate["source_count"], 11)
        self.assertEqual(aggregate["minimum_source_support"], 2)
        self.assertEqual(len(aggregate["stable_patterns"]), 36)
        self.assertEqual(len(aggregate["source_specific_patterns"]), 56)
        self.assertEqual(
            {
    ...
    def test_committed_aggregate_matches_the_eleven_formal_studies(self) -> None:
        repository_root = Path(__file__).resolve().parents[1]
