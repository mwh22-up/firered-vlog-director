...
class TechniqueLearningTests(unittest.TestCase):
    ...
    def test_fifteen_formal_studies_keep_the_learning_baseline(self) -> None:
        ...
        self.assertEqual(len(studies), 15)
        self.assertEqual(observation_count, 275)
        self.assertEqual(len(unique_keys), 108)
        ...
        self.assertEqual(aggregate["source_count"], 15)
        self.assertEqual(aggregate["minimum_source_support"], 2)
        self.assertEqual(len(aggregate["stable_patterns"]), 44)
        self.assertEqual(len(aggregate["source_specific_patterns"]), 64)
        self.assertEqual(
            {
    ...
    def test_committed_aggregate_matches_the_fifteen_formal_studies(self) -> None:
        repository_root = Path(__file__).resolve().parents[1]
