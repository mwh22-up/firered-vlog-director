...
- 不依赖真实素材的单元测试与 FFmpeg `lavfi` 合成媒体集成测试。
...
`test` extra 会安装单元测试、参考分析、JSON Schema 门禁和固定版本的便携 FFmpeg 所需依赖；测试会校验全部 8 个 Schema、仓库中的正式 JSON 产物，并用临时 `lavfi` 音视频实际执行基础增强渲染。合成媒体只存在于测试临时目录，不进入 Git。
